...
description: 'LCP 개선을 위한 next/image 활용법과 실제 성능 최적화 경험을 공유합니다.'
pubDate: 2025-08-04
tags: ['Next.js', '성능최적화', '이미지최적화', 'LCP']
...
웹 서비스의 성능 지표를 개선하며, LCP 개선을 위해 활용했던 `next/image` 최적화와 `next/image`만으로는 해결되지 않았던 상황에서 어떤 보완 전략을 적용했는지도 함께 공유하려 합니다.
...
브라우저가 이미지를 요청할 때 Request Header의 `Accept` 필드에 지원 포맷을 명시합니다.
